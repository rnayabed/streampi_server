...
public class OBSStartStopStreamingConfig implements Initializable {
    ...
    @FXML
    private JFXToggleButton startStopStreamingToggleButton;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        startStopStreamingToggleButton.setOnMouseClicked(event -> {
            if(startStopStreamingToggleButton.isSelected())
            {
                startStopStreamingLabel.setText("Start Streaming");
            }
            else
            {
                startStopStreamingLabel.setText("Stop Streaming");
            }
        });
        ...
        if(dashboardController.actionConfigType == 2)
        {
            ...
            headingLabel.setText("Modify OBS Studio (Start/Stop Streaming) Action");
            ...
            for(int i = 0;i<dashboardController.actions.length;i++)
            {
                ...
                if(eachAction[0].equals(dashboardController.selectedActionUniqueID))
                {
                    ...
                    String startStopStreamingValue = eachAction[3];
                    if(startStopStreamingValue.equals("1"))
                    {
                        startStopStreamingToggleButton.setSelected(true);
                        startStopStreamingLabel.setText("Start Streaming");
                    }
                    else if(startStopStreamingValue.equals("2"))
                    {
                        startStopStreamingToggleButton.setSelected(false);
                        startStopStreamingLabel.setText("Stop Streaming");
                    }
    ...
    int i;
    String startStopStreamingValue;
    @FXML
    public void addButtonClicked()
    {
        String actionCasualName = actionCasualNameField.getText();
        if(startStopStreamingToggleButton.isSelected())
            startStopStreamingValue = "1";
        else
            startStopStreamingValue = "2";
            ...
            protected Void call() {
                try
                {
                    ...
                    if(dashboardController.actionConfigType == 1)
                    {
                        ...
                        oldActions[i][2] = "obs_start_stop_streaming";
                        String toWrite = startStopStreamingValue;
